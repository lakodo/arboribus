...
def resolve_patterns(source_dir: Path, patterns: List[str], filters: Optional[List[str]] = None) -> List[Path]:
...
@app.command()
def init(
    source: str = typer.Option(..., "--source", "-s", help="Source root directory"),
...
def list_rules(
    ...
    for target_name, target_config in config["targets"].items():
        ...
        for pattern in target_config["patterns"]:
            ...
            matched_str = "\n".join([str(d.relative_to(source_dir)) for d in matched_dirs]) or "None"
            target_paths = "\n".join([
